...
def part1_encoder():
    ...
    def dct2D(x):
        ...
        transposed_dct = np.transpose(temp_dct)
        
        temp_result = dct(transposed_dct,type=2,norm = 'ortho')
    ...
    for i in range(nbh):
        ...
        for j in range(nbw):
            ...
            reshaped_Y = inverse_zigzag(reordered_Y, block_size, block_size)###### Your code here ######
            reshaped_Cb = inverse_zigzag(reordered_Cb, block_size, block_size)###### Your code here ######
            reshaped_Cr = inverse_zigzag(reordered_Cr, block_size, block_size)###### Your code here ######
            ...
            ###### Your code here ####
            non_zeros_original = np.count_nonzero(YDCT)
            non_zeros_quantized = np.count_nonzero(quantized_YDCT)
            total_non_zeros_before += non_zeros_original
            total_non_zeros_after += non_zeros_quantized
    ...
    ###### Your code here ######
    np.savetxt('size.txt', [height, width, c, block_size], fmt='%d')

    # TODO: Calculate percentage of pixel locations with non-zero values before and after to measure degree of compression 
    percentage_non_zeros_before = total_non_zeros_before / (height * width)
    percentage_non_zeros_after = total_non_zeros_after / (height * width)
    #degree_of_compression = 100 * (1 - percentage_non_zeros_after / percentage_non_zeros_before)

    # Print statements as shown in eClass
    ###### Your code here ######
    print('Percentage of non-zero elements in Luma channel:')
    print(f"Before compression: {percentage_non_zeros_before:.2%}")
    print(f"After compression: {percentage_non_zeros_after:.2%}")
...
def part2_decoder():
    ...
    def idct2D(x):
        ###### Your code here ######
        intermediate = idct(x, norm='ortho', axis=0)
        result = idct(intermediate, norm='ortho', axis=1)
        return result
    ...
    ###### Your code here ######
    h, w, c, block_size = np.loadtxt('size.txt', dtype=int)
